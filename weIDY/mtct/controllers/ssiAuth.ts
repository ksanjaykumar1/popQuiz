...
import { agent } from '../integration/integration';
import Listener from '../utils/Listner';
...
const register = async (req: express.Request, res: express.Response) => {
  ...
  listner.createConnectionListnerSignUp(oob, email);
  const invitationUrl = oob.outOfBandInvitation.toUrl({
    domain: 'https://example.org',
  });
  qrcode.generate(invitationUrl, { small: true });
  res.status(StatusCodes.CREATED).json({
    msg: 'Scan the QR code or use invitation URL to connect and get Login credential Issued as VC',
